...
results_folder = './wwt-conductivity-results'
...
def nn_model(feature_train):
    device_name = "/GPU:0" if tf.test.gpu_device_name() == "/device:GPU:0" else "/CPU:0"
...
def plot_results(label_train,label_train_pred,label_val,label_val_pred,label_test,label_test_pred,r2_score_train,r2_score_val,r2_score_test,figure_name,title='no title'):
    ...
    plt.title(title)
    plt.legend()
    plt.savefig(f"./wwt-conductivity-results/{figure_name}.png")
    plt.show()
    
#%% Import data
path = './data/water-treatment.data'
...
plot_results(label_train,label_train_pred,label_val,label_val_pred,label_test,label_test_pred,r2_score_train,r2_score_val,r2_score_test,title='Without Hyperparameter Tuning (ANN)',figure_name='without-tuning-ann')
...
def ann_model_hppt(space): 
    with tf.device(device_name):
        ...
        model.add(Dropout(rate=space['rate1']))
        model.add(Dense(units=1, activation='relu', kernel_initializer=GlorotUniform(seed=0)))
        model.compile(loss=space['loss'], optimizer=tf.keras.optimizers.legacy.Adam(learning_rate=space['learning_rate']))
...
with tf.device(device_name):
    print(f"training on a {device_name}")
...
#%% plotting
plot_results(label_train,label_train_pred_hppt,label_val,label_val_pred_hppt,label_test,label_test_pred_hppt,r2_score_train_hppt,r2_score_val_hppt,r2_score_test_hppt,title='With Hyperparameter Tuning (ANN)',figure_name='with-tuning-ann')
...
plt.tight_layout()
plt.savefig('./wwt-conductivity-results/feature-importance-ann.png')
plt.show()
